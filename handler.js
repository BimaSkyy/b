const { Logs, Scraper, InvCloud } = new(require('@neoxr/wb'))
...
module.exports = async (client, ctx) => {
   ...
   try {
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', '🚩 Bot sudah habis masanya/kedaluarsa di group ini.', null, {
      ...
      if (users && (new Date * 1) >= users.expired && users.expired != 0) {
         return client.reply(users.jid, Func.texted('italic', '🚩 Paket premium kamu telah kedaluarsa, Terima kasih telah menggunakan layanan kami.')).then(async () => {
            users.premium = false
            users.expired = 0
      ...
      if (prefix && !commands.includes(command) && matcher.length > 0 && !setting.self) {
         if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `🚩 Perintah yg kamu gunakan salah, coba salah satu rekomendasi  ini : \n\n${matcher.map(v => '➠ *' + (prefix ? prefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
      }
      if (body && prefix && commands.includes(command) || body && !prefix && commands.includes(command) && setting.noprefix || body && !prefix && commands.includes(command) && env.evaluate_chars.includes(command)) {
         if (setting.error.includes(command)) return client.reply(m.chat, Func.texted('bold', `🚩 Perintah _${(prefix ? prefix : '') + command}_ dinonaktifkan.`), m)
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'menfess', 'scan', 'verify', 'payment', 'premium'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) {
               client.sendMessageModify(m.chat, `⚠️ bot sedang dalam mode khusus groups, atau sedang ada perbaikan, silakan join group untuk mendapatkan informasi.`, m, {
                  largeThumb: true,
            ...
            if (cmd.restrict && !isPrem && !isOwner && text && new RegExp('\\b' + setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `⚠️ Anda melanggar *Syarat & Ketentuan* penggunaan bot dengan menggunakan kata kunci yang masuk daftar hitam, sebagai hukuman atas pelanggaran Anda diblokir dan dibanned.`, m).then(() => {
                  users.banned = true
               })
               continue
            }
            ...
            if (cmd.limit && users.limit < 1) {
               client.reply(m.chat, `⚠️ Anda telah mencapai batas yg di tentukan, Untuk itu limit Anda akan diatur ulang pada pukul 00.00\n\nUntuk mendapatkan limit lebih besar, tingkatkan ke paket premium.`, m).then(() => users.premium = false)
               continue
            }
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, Func.texted('bold', `⚠️ Anda tidak memiliki cukup limit untuk menggunakan fitur ini.`), m)
                  continue
               }
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && setting.groupmode && !['system_ev', 'menfess_ev', 'chatbot', 'auto_download'].includes(name) && !isPrem) return client.sendMessageModify(m.chat, `⚠️ bot sedang dalam mode khusus groups, atau sedang ada perbaikan, silakan join group untuk mendapatkan informasi.`, m, {
               largeThumb: true,
            ...
            if (event.limit && !event.game && users.limit < 1 && body && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v))) return client.reply(m.chat, `⚠️ Anda telah mencapai batas yg di tentukan, Untuk itu limit Anda akan diatur ulang pada pukul 00.00\n\nUntuk mendapatkan limit lebih besar, tingkatkan ke paket premium.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
            ...
            event.async(m, { client, body, prefixes, groupMetadata, participants, users, chats, groupSet, setting, isOwner, isAdmin, isBotAdmin, plugins, blockList, env, ctx, store, Func, scrap})
         }
      }
   } catch (e) {
