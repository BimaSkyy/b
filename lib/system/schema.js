module.exports = (m, env) => {
   ...
   if (user) {
      if (!('bio' in user)) user.bio = 'kamu gada bio'
      if (!('username' in user)) user.username = 'kamu gada nama'
...
if (!isNumber(user.buku)) user.buku = 0
      if (!isNumber(user.cahaya)) user.cahaya = 0
   ...
   if (m.isGroup) {
      ...
      if (group) {
         ...
         if (!('antilinknokick' in group)) group.antilinknokick = false
   ...
   if (setting) {
     ...
     if (!isNumber(setting.receiveSize)) setting.receiveSize = 0
      ...
      if (!('sk_pack' in setting)) setting.sk_pack = 'Made By'
      if (!('sk_author' in setting)) setting.sk_author = 'AlesyaBot 083132988099'
      if (!('self' in setting)) setting.self = false
      if (!('verify' in setting)) setting.verify = false
      if (!('viewonce' in setting)) setting.viewonce = false
      if (!('mimic' in setting)) setting.mimic = []
      if (!('hidden' in setting)) setting.hidden = []
      if (!('limiter' in setting)) setting.limiter = false
      ...
      if (!('owners' in setting)) setting.owners = ['6283132988099']
      if (!isNumber(setting.lastReset)) setting.lastReset = new Date * 1
      if (!('msg' in setting)) setting.msg = 'Hai +tag 👋🏻\nselamat datang di bot WhatsApp alesya, kamu bisa menggunakan saya dengan fitur² yang saya punya dan itu geratis kawan.'
      if (!isNumber(setting.menuStyle)) setting.menuStyle = 4
      if (!('cover' in setting)) setting.cover = 'https://i.ibb.co/HqJs0NW/image.jpg'
      if (!('link' in setting)) setting.link = 'https://whatsapp.com/channel/0029VaAQ0dF7T8bcL5yjvq1s'
   } else {
      global.db.setting = {
         autodownload: true,
         debug: false,
         ...
         groupmode: false,
         sk_pack: 'Made By',
         sk_author: 'AlesyaBot 083132988099',
         self: false,
         verify: false,
         viewonce: false,
         mimic: [],
         ...
         online: true,
         onlyprefix: '+',
         owners: ['6283132988099'],
         lastReset: new Date * 1,
         msg: 'Hai +tag 👋🏻\nselamat datang di bot WhatsApp alesya, kamu bisa menggunakan saya dengan fitur² yang saya punya dan itu geratis kawan.',
         menuStyle: 4,
         cover: 'https://i.ibb.co/HqJs0NW/image.jpg',
         link: 'https://whatsapp.com/channel/0029VaAQ0dF7T8bcL5yjvq1s'
      }
   }
}
