...
exports.run = {
    ...
    async: async (m, {
        ...
        try {
            ...
            if (args.length && (args[0] === '+log' || args[0] === '-log')) {
                command = args.shift(); // Shift the command from args
            }    
            ...
            if (command == '+log') {
                ...
                let [type, logText] = text.split('|').map(str => str.trim()); // Trim each part
                ...
                logs[id] = {
                    log: Func.texted('bold', `${typeLabel}`) + ` ${logText}`, // Add a space after typeLabel
                    at: new Date() * 1
                };
