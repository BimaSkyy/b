   ...
   async: async (m, {
      ...
      try {
         ...
         if (command == '+prem') {
            ...
            client.reply(m.chat, data.premium ? Func.texted('bold', `🚩 Succesfully added 30 days premium access for @${user.replace(/@.+/, '')}.`) : Func.texted('bold', `🚩 Successfully added @${user.replace(/@.+/, '')} to premium user.`), m).then(() => data.premium = true)
