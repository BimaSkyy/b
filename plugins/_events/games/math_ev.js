         ...
         if (m.quoted && /Berapa hasil dari/i.test(m.quoted.text)) {  
            ...
            if (m.text == math.result) {
               ...
               if (--client.math[id][2] == 0) {
                  clearTimeout(client.math[id][3])
                  delete client.math[id]
                  await client.reply(m.chat, `🚩 _Permainan berakhir karena telah 3x menjawab salah, jawabannya adalah_ : *${math.result}*`, m)
               } else await client.reply(m.chat, '*X*', m) // client.reply(m.chat, Func.texted('bold', `${client.math[id][2]} chances to answer.`), m)}
            }
         }
      } catch (e) {
