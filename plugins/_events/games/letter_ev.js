         ...
         if (m.quoted && /letskip/i.test(m.quoted.text)) {  
            ...
            if (body == client.letter[id][1].answer) {
               ...
               if (--client.letter[id][2] == 0) {
                  clearTimeout(client.letter[id][3])
                  await client.reply(m.chat, `🚩 _Permainan berakhir karena telah 3x menjawab salah, jawabannya adalah_ : *${client.letter[id][1].answer}*`, m)
                     delete client.letter[id]
               } else await client.reply(m.chat, 'selamat kamu salah tlol✓', m) // client.reply(m.chat, Func.texted('bold', `${client.letter[id][2]} chances to answer.`), m)}
            }
         }
      } catch (e) {
