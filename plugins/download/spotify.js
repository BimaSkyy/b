   ...
   async: async (m, {
      ...
      try {
         ...
         if (args[0].match('open.spotify.com') && /track/.test(args[0])) {
            var json = await Func.fetchJson('https://tiktod.eu.org/spotify?q=' + args[0])
            if (!json.status) return client.reply(m.chat, Func.jsonFormat(json), m)
            let caption = `乂  *S P O T I F Y*\n\n`
            caption += `	◦  *Title* : ${json.result.info.title}\n`
            caption += `	◦  *Artis* : ${json.result.info.artist}\n`
            caption += `	◦  *Album* : ${json.result.info.album}\n`
            caption += `	◦  *Tahun* : ${json.result.info.year}\n\n`
            caption += global.footer
            client.sendMessageModify(m.chat, caption, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer(json.result.info.albumCoverURL)
            }).then(async () => {
               client.sendFile(m.chat, json.result.url, json.result.info.title + '.mp3', '', m, {
                  document: true,
                  APIC: await Func.fetchBuffer(json.result.info.albumCoverURL)
               })
            })
